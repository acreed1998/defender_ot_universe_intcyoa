  ...
  if ($('#section-Events').length !== 0) {
    ...
    _.each(data.unique_rewards.choices, (choice, choiceKey) => {
      ...
      if (row === 2 || row === 3 || row === 5) {
        $(`#section-Events-div-uniqueRewards-row-${row - 1}`).removeClass('choices').append($('<div>').addClass('choice centered').css({ 'width': `75%`}).attr({'id' : `section-Events-div-${choiceKey}`}).html(choice.text));
      } else {
...
function addRowC(rowNum) {
  $('#section-Events-div-uniqueRewards').append($('<div>').addClass('choices centeredChoice').css({'width' : '100%'}).attr({ 'id': `section-Events-div-uniqueRewards-row-${rowNum}` }));
  rowNum = rowNum + 1;
  return rowNum;
}